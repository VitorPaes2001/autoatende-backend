...
const Attendance = () => {
  ...
  }, [session?.access_token]); // eslint-disable-line react-hooks/exhaustive-deps -- initial load reacts to auth; adding the render-local loader identity would refetch after each state update

  
  const currentUserId = session?.user?.id || null;

  const queueSourceRows = rows;
